<?php

/* partical/nav.phtml */
class __TwigTemplate_f0336ca9ead8908ae98a3560be89a034 extends Twig_Template
{
    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->parent = false;

        $this->blocks = array(
        );
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        // line 1
        echo "\t<nav class=\"top-bar\">
  <ul class=\"title-area\">
    <!-- Title Area -->
    <li class=\"name\">
      <h1><a href=\"/\"><img  class='logo' src='/img/logo.png'></a></h1>
    </li>
    <li class='actions'>
      <ul class=\"actionbar\">
      \t<li><a href=\"javascript:void(0)\" class=\"new\" id=\"action-new\"><i class=\"icon-file-alt\"></i></a></li>
        <li><a href=\"javascript:void(0)\" class=\"save\" id=\"action-save\"><i class=\"icon-save\"></i></a></li>
        <li><a href=\"javascript:void(0)\" class=\"style\" id=\"action-style\"><i class=\"icon-columns\"></i></a></li>
      </ul>
    </li>
    <li class=\"toggle-topbar menu-icon\"><a href=\"#\"><span>Menu</span></a></li>
  </ul>
  <section class=\"top-bar-section\">
    <ul class=\"left\">
      <li><a class='write' href=\"write\"><span><i class=\"icon-pencil\"></i> Write</span></a></li>
      <li class=\"split\"></li>
      <li><a class='browse' href=\"browse\"><span>Browse</span></a></li>
      <li class=\"split\"></li>
      <li><a class='explorer' href=\"explore\"><span>Explore</span></a></li>
      <li class=\"split\"></li>
      <li><a class='about' href=\"about\"><span>About</span></a></li>
    </ul>
    <ul class=\"right\">
      <li>
        <a class='avatar template' data-template='logined'>
          <script type=\"text/template\">
            <% if (login) { %>
            <span>Ruly</span>
            <img src='img/avatar.jpg'>
            <span><i class=\"icon-angle-down\"></i></span>
            <% } else { %>
            <span><i class=\"icon-signin\"></i> Log In</span>
            <% } %>
          </script>
        </a>

        <ul class=\"action-dropdown template\" data-template='logined-drop'>
          <script type=\"text/template\">
            <span class=\"arrow\"></span>
            <li class='top'>
              <img src='img/avatar1.jpg'>
              <span class=\"user-name\">Ruly</span>
              <span class=\"user-email\">surgesoft@gmail.com</span>
            </li>
            <li class='normal'><a href='#'><i class=\"icon-info\"></i>Manager Profile</a></li>
            <li class='normal'><a href='#'><i class=\"icon-envelope\"></i>Mail</a></li>
            <li class=\"split\"></li>
            <li class='normal'><a href='#'>Invite someone to CW</a></li>
            <li class=\"split\"></li>
            <li class='normal'><a href='#'><i class=\"icon-question\"></i>Help</a></li>
            <li class=\"split\"></li>
            <li class='normal'><a href='/logout'><i class=\"icon-signout\"></i>Log Out</a></li>
            <span class=\"shadow shadow-left\"></span>
            <span class=\"shadow shadow-right\"></span>
            <span class=\"shadow shadow-bottom\"></span>
          </script>
        </ul>
      </li>
    </ul>
  </section>
</nav>";
    }

    public function getTemplateName()
    {
        return "partical/nav.phtml";
    }

    public function getDebugInfo()
    {
        return array (  19 => 1,);
    }
}
